                  ...
                  attachBody match {
                    ...
                    case Failure(exception) =>
                      ...
                      exception.toString match {
                        case "java.net.SocketTimeoutException: connect timed out" => Some(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, "Unable to connect to the endpoint in a timely manner", 10))
                        case "java.net.ConnectException: Connection refused (Connection refused)" => Some(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, "Check to make sure the service exists", 20))
                        case "java.net.SocketTimeoutException: Read timed out" => Some(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, "Unable to get the body from the endpoint in a timely manner", 30))
                        case _ => Some(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, exception.getMessage))
                      }
                  }
                }
                else None
                    ...
                    case Success(_) =>
                      ...
                      Try(openConn.getResponseCode) match {
                        case Success(respCode) =>
                          ...
                          val response: HttpClientResponse = HttpClientResponse(timestamp, url, method, body, combinedHeaders, metaData, respCode, responseBody.replaceAll("[\n\r]", " "), removeNullFromHeaders(openConn), (t1 - t0).toInt)
                          Try(openConn.getInputStream.close())
                          Right(response)
                        case Failure(ex) =>
                          Try(openConn.getErrorStream.close())
                          Left(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (System.currentTimeMillis() - t0).toInt, ex.toString, ex.getMessage))
                      }
                    case Failure(exception) =>
                      val exceptionTimeMark: Long = System.currentTimeMillis()
                      exception.toString match {
                        case "java.net.SocketTimeoutException: connect timed out" => Left(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, "Unable to connect to the endpoint in a timely manner", 10))
                        case "java.net.ConnectException: Connection refused (Connection refused)" => Left(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, "Check to make sure the service exists", 20))
                        case "java.net.SocketTimeoutException: Read timed out" => Left(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, "Unable to get the body from the endpoint in a timely manner", 30))
                        case _ => Left(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (exceptionTimeMark - callCreateTimeMark).toInt, exception.toString, exception.getMessage))
                      }
                  }
              }
            case _ =>
              //Never seen this happen yet...
              Left(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (System.currentTimeMillis() - callCreateTimeMark).toInt, "Error while attempting to get ConnectionObject"))
          }
        case Failure(ex) => Left(HttpClientError(timestamp, url, method, body, combinedHeaders, metaData, (System.currentTimeMillis() - callCreateTimeMark).toInt, ex.toString, ex.getMessage))
      }
    }
